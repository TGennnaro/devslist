...
import { pgTable, pgView, serial, text, uniqueIndex, integer, json, date, varchar, boolean, timestamp, real, customType} from 'drizzle-orm/pg-core';
...
export const Users = pgTable(
	'users',
	{
        ...
        resume: bytea('resume').notNull(),
        about: text('about').notNull(),
        dob: date('dob').notNull(),
        roles: text('roles').notNull().$type<"user" | "employer">(),
        githubID: text('github_id').notNull(),
        gender: boolean('gender').notNull(),
        veteranStatus: boolean('veteran_status').notNull(),
...
export const Jobs = pgTable(
	'jobs',
	{
		...
		companyid: text('companyID').notNull().references(() => Company.companyid),
...
export const Application = pgTable(
	'application',
	{
		...
		userid: text('userid').notNull().references(() => Users.userid),
		jobid: text('jobid').notNull().references(() => Jobs.jobid),
...
export const link = pgTable(
	'link',
	{
		...
		userid: text('userid').notNull().references(() => Users.userid).primaryKey(),
...
export const Connection = pgTable(
	'connection',
	{
		...
		user1id: text('user1id').notNull().references(() => Users.userid),
		user2id: text('user2id').notNull().references(() => Users.userid),
		status: integer('status').notNull(),
	},
	(connection) => {
		return {
